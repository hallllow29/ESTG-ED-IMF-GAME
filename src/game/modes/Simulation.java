 ...
 * mission-based game. It provides methods and mechanisms to handle player actions,
 * enemy movements, and overall game progression. The class manages the state of
 * the simulation, including the battlefield, player status, mission objectives, and
 * various other scenarios.
 */
public abstract class Simulation {

	/**
	 * Represents the mission instance associated with the simulation.
	 * Provides essential details and components of the mission, including
	 * battlefield information, targets, enemies, items, and entry/exit points.
	 * This variable works as a core element for managing and executing
	 * the mission's objectives and scenarios during the simulation.
	 */
	private final Mission mission;
	/**
	 * Represents the battlefield used in the simulation, which is a network of
	 * interconnected rooms. The battlefield is managed as a custom network of
	 * rooms and is used to simulate the environment in which game scenarios,
	 * including player and enemy movements and interactions, take place.
	 */
	private CustomNetwork<Room> battlefield;
	/**
	 * Represents the player participating in the simulation.
	 * This variable holds a reference to the Player instance used during the simulation,
	 * encapsulating the player's attributes, position, inventory, and actions.
	 * It is declared as final, ensuring that the player reference cannot be re-assigned.
	 */
	private final Player player;

	/**
	 * Represents the state of the game, indicating whether the game has ended.
	 * This variable is used to manage the flow of the simulation by checking
	 * if the game conditions have been met to mark its completion.
	 *
	 * A value of {@code true} indicates that the game is over, and no further
	 * actions or turns can be taken. Conversely, a value of {@code false}
	 * indicates that the game is still ongoing.
	 */
	private boolean gameOver;

	/**
	 * Represents the starting room or initial point of entry in the simulation.
	 * The entryPoint serves as the location where the simulation begins,
	 * typically positioning the player at the start of the gameplay or mission.
	 */
	...
	/**
	 * Represents the state of whether the player is currently returning
	 * to the extraction point in the simulation.
	 * This variable is used to track the phase of the mission where
	 * the player has completed their main objective and is navigating
	 * towards the designated exit point.
	 */
	private boolean returningToExtraction;

	/**
	 * Represents the current turn in the simulation, determining whose actions
	 * will be executed in the ongoing game cycle.
	 *
	 ...
	 *
	 * This variable is used to track the ongoing phase or situation of the gameplay
	 * or simulation. It determines the current scenario logic, behavior, or sequence
	 * of actions in the simulation, and it can trigger the execution of specific conditions
	 * or transitions to subsequent scenarios.
	 */
	private ScenarioNr currentScenario;

	/**
	 * Indicates whether the mission within the simulation has been successfully completed.
	 * This variable is set to true when all conditions for mission success are achieved,
	 * such as reaching a target, securing objectives, or meeting scenario-specific goals.
	 * It is false by default and reflects the current status of the mission during the simulation.
	 */
	private boolean missionAccomplished;

	/**
	 * Represents the next objective or target room in the simulation.
	 * This variable keeps track of the Room instance that is the player's
	 * current target or goal, guiding the progression of the simulation's
	 * objectives. Updates occur based on the game's scenarios, player actions,
	 * or mission stages.
	 */
	private Room nextObjective;

	/**
	 * A collection of enemies currently present within the simulation.
	 * This LinkedList stores instances of the {@code Enemy} class, where each enemy
	 * represents a hostile entity that the player may encounter during the game.
	 * The list is used to manage and track the enemies' states, positions, and interactions
	 * within the game's environment.
	 */
	private LinkedList<Enemy> enemies;

	/**
	 * Represents the room that is geographically closest to the player's
	 * current position and contains items. It is used during the simulation
	 * to identify and interact with nearby rooms of interest that have
	 * collectible items. The room may change dynamically based on the player's
	 * movement and gameplay actions.
	 */
	private Room closestItem;

	/**
	 * Represents the report associated with the simulation.
	 * This report maintains the details of the simulation lifecycle,
	 * such as mission status, player performance, enemies confronted,
	 * and paths traversed during the mission.
	 */
	private final Report report;

	/**
	 * Constructs a Simulation instance which initializes the game with the given mission, player, and report.
	 *
	 * @param mission The mission associated with the simulation, containing details about the battlefield, enemies, and items.
	 * @param player The player participating in the simulation.
	 * @param report The report that will record the progress and outcomes of the simulation.
	 */
	...
	/**
	 * Moves the player to a new position within the simulation.
	 * This method is responsible for determining and executing the player's
	 * movement within the battlefield based on the current state of the game.
	 *
	 * @throws ElementNotFoundException if a required element, such as the target
	 *         room or path, is not found during the movement process.
	 * @throws EmptyCollectionException if the collection used for determining the
	 *         player's movement, such as paths or possible moves, is empty.
	 */
	public abstract void movePlayer() throws ElementNotFoundException, EmptyCollectionException;

	/**
	 * Initiates and oversees the core gameplay loop within the simulation.
	 * This method is responsible for managing the sequential flow of a simulation
	 * session, executing actions and managing the state of the game until its
	 * conclusion. It integrates player actions, game state updates, and progression
	 * within the mission being simulated.
	 *
	 * @throws ElementNotFoundException if a required component within the simulation,
	 *         such as a target, enemy, or room, cannot be located during the execution
	 *         of the game logic.
	 * @throws EmptyCollectionException if an operation within the simulation attempts
	 *         to access or manipulate an empty collection critical to the gameplay
	 *         mechanics (e.g., paths, enemies, or items).
	 */
	public abstract void game() throws ElementNotFoundException, EmptyCollectionException;

	/**
	 * Manages the core flow of the game's turn-based mechanics. This method is responsible for
	 * alternating between the player's turn and the enemy's turn until the game is marked as over.
	 * It checks for mission success conditions and updates the game report upon completion.
	 *
	 * @throws EmptyCollectionException if an operation within the player's or enemy's turn
	 *         attempts to access or manipulate an empty collection critical to the game flow.
	 * @throws ElementNotFoundException if a required element (such as a path, item, or enemy)
	 *         cannot be located during the execution of the player's or enemy's turn.
	 */
	...
	/**
	 * Executes the player's turn in the current simulation. This method is responsible for
	 * managing the logic of the player's actions and their interactions with the game environment,
	 * including handling scenarios based on the player's position, confronting enemies, and moving
	 * to the next location.
	 *
	 * If the player is in a room without enemies or in a specific scenario, the player moves to
	 * another room as determined by the game's logic. Otherwise, additional scenario-based logic
	 * is triggered. After the primary actions of the player's turn, scenario-specific
	 * situations and cases are evaluated.
	 *
	...
	/**
	 * Renders the automatic simulation by setting up the next objective, determining the best
	 * entry point, and positioning the player within the game. This method manages the initial
	 * setup and state configuration for an automated simulation process.
	 *
	 * @param player The player participating in the simulation. The player's position is updated
	 *               to the calculated entry point as part of the simulation setup.
	 * @param target The target object specifying the next objective room for the simulation.
	 *               The objective room is set to guide the player's progression.
	 * @throws ElementNotFoundException If the entry point or required simulation element cannot be
	 *                                  determined or located during initialization.
	 */
	...
	/**
	 * Prepares and starts a manual simulation for the given target room in the game.
	 * This involves setting the next objective, initializing the game state, and
	 * configuring the appropriate turn structure for the simulation.
	 *
	 * @param target The room that the player is tasked to move toward as the next objective in the manual simulation.
	 */
	 ...
	 *
	 * @return the report object that contains information about the progress,
	 *         outcomes, and details of the simulation.
	 */
	...
	/**
	 * Retrieves the battlefield associated with the current simulation.
	 * The battlefield is represented as a custom network of rooms where the simulation takes place.
	 *
	 ...
	 *                 If true, the simulation is marked as completed.
	 */
	 ...
	 * The current scenario represents the specific phase or condition
	 * the simulation is currently in, which may influence gameplay,
	 * mission objectives, or other aspects of the simulation process.
	 *
	 * @return the current scenario of type ScenarioNr that corresponds to the
	 *         current phase or state of the simulation.
	 */
	...
	/**
	 * Retrieves the list of enemies currently present in the simulation.
	 * The list contains all enemies currently active and relevant within the game's battlefield.
	 *
	...
	/**
	 * Retrieves the next objective room for the simulation.
	 * The next objective represents the target room that the player
	 * is currently tasked to move toward or interact with during the mission.
	 *
	...
	/**
	 * Sets the entry point for the simulation.
	 * The entry point represents the starting room or location
	 * in the battlefield where the simulation begins.
	 *
	 ...
	 *
	 * @param nextObjective The Room object representing the next objective to be assigned.
	 */
	...
	/**
	 * Executes the enemy's turn in the game. This method is called during the
	 * game loop to process enemy actions based on the current scenario.
	 *
	 * The method performs the following steps:
	 * 1. If the current scenario is `ScenarioNr.TWO`, it moves the player
	 *    and sets the turn to `ENEMY`.
	...
	/**
	 * Handles the scenarios and situations of a player's current position.
	 * Retrieves the player's current position, checks if the position contains enemies,
	 * and processes the room's situation based on the findings.
	 * This method is intended to manage game logic related to the player's current room and potential threats.
	 */
	...
	/**
	 * Determines the situation of the room based on the player's position and enemy presence,
	 * and sets the next scenario accordingly.
	 *
	 ...
	 *
	 * @param missionAccomplished a boolean flag indicating whether the mission has been accomplished (true) or not (false)
	 */
	 ...
	 * @throws ElementNotFoundException if an expected element is missing during scenario execution.
	 * @throws EmptyCollectionException if an operation is attempted on an empty collection during scenario execution.
	 */
	...
	/**
	 * Executes the Scenario UM sequence for the game.
	 * The method manages the scenario where the player enters a room,
	 * confronts enemies, and collects items if all enemies are defeated.
	 * It displays messages for the player's actions, the enemy's actions,
	 * and the outcome of the current turn. The method also determines
	 * the next turn in the game based on whether enemies remain in
	 * the room or not.
	 *
	 */
	...
	/**
	 * Manages the flow of events in the second scenario (Scenario DOIS).
	 * This method handles the interactions and updates that occur within the scenario,
	 * including player's actions, item collection, and enemy behavior.
	 */
	...
	/**
	 * Executes the logic for scenario three (TRES) within the game. This method
	 * handles player and enemy interactions, turn-based confrontation sequences,
	 * and checks for victory or defeat conditions. The following actions occur:
	 *
	 * 1. Initializes the scenario with introductory messages and enemy confrontation setup.
	 * 2. Executes the combat loop while the player is alive and enemies are present in the room:
	...
	/**
	 * Represents the implementation of the Scenario QUATRO game logic.
	 * This scenario handles the following sequence of events:
	 ...
	 * - Attempts to call the player's use of a MediKit. If the MediKit
	 *   is unavailable, an EmptyCollectionException is caught and its
	 *   message is logged via standard error output.
	 * - Displays an end message signifying the conclusion of the scenario.
	 * - Sets the next turn to the enemy.
	 *
	 * Method execution is dependent on external systems such as the Display class
	 * for constructing messages and the Player instance for invoking actions.
	 */
	 ...
	 *
	 * This method represents a specific scenario where the player enters a room
	 * and confronts potential enemies. The following actions are performed:
	 *
	 * - The player's current position is retrieved.
	 * - A series of informational messages is displayed to describe the events
	 *   occurring during the scenario, including the player's actions, enemy actions, and the
	 *   overall scenario progression.
	 * - The player initiates a confrontation.
	 * - Checks are made to determine if enemies are still present in the room.
	 * - If enemies remain, appropriate messages are displayed describing the state of the enemies
	 *   and their actions, such as taking their turn or moving.
	...
	/**
	 * Executes the scenarioSEIS sequence in the game.
	 * This method handles the flow of events when the player enters
	 * the scenario, including interactions with the environment
	 * and determining the outcome of the scenario.
	 *
	 * @throws ElementNotFoundException if an element required for the
	 *         execution of the scenario is not found.
	 */
	 ...
	 *
	 * This method marks the mission objective as secured, sets the returning-to-extraction
	 * flag, and identifies the best extraction point based on the player's current position.
	 * It then updates the mission's next objective to the determined extraction point.
	 *
	 * @throws ElementNotFoundException if the method is unable to find a valid extraction point.
	 */
	 ...
	 * @return true if the player's current position is the same as the target position, otherwise false
	 */
	...
	/**
	 * Determines if the mission is accomplished based on the player's status,
	 * the mission's target security, and the player's current position relative
	 * to the next objective.
	 *
	 * @return true if the player is alive, the mission target is secured,
	 *         and the player is at the next objective; false otherwise.
	 */
	 ...
	 * entry/exit points provided by the mission and determines the optimal entry point
	 * by iterating over the battlefield network.
	 *
	 * @return The Room object representing the best entry point with minimal damage
	 *         to the next objective. Returns null if no suitable entry point is found.
	 ...
	 *
	 * @return true if the player has a current health value less than or equal to a critical threshold
	 *         and possesses a recovery item; false otherwise.
	 */
	...
	/**
	 * Moves all enemies that are not in the same room as the player.
	 * This method iterates through the list of enemies and checks their
	 * positions. If an enemy is located in a different room than the
	 * player's current position, the enemy is moved by invoking the
	 * moveEnemy method.
	 */
	...
	/**
	 * Moves the specified enemy to a new position in a randomly selected, valid room.
	 * The method first determines the possible moves for the enemy, then updates
	 * the enemy's position and adjusts the status of the rooms involved accordingly.
	 *
	 * @param enemy the enemy to be moved. The enemy's current position is used to
	 *              determine possible moves, and its position is updated to a new
	 *              room if valid moves exist.
	 */
	...
	/**
	 * Determines the possible moves from the specified room within a certain range
	 * based on a breadth-first search traversal of the battlefield.
	 *
	...
	/**
	 * Collects items located in the specified room and processes their pickup by the player.
	 * If the player's backpack is not full, items such as MediKit are added to the backpack,
	 * and Kevlar is equipped directly. Updates the player's position and item collection state.
	 *
	 * @param room The room where items are being gathered. Only items located in this room are processed.
	 * @return A string containing messages about the items spotted and actions performed, such as
	 *         adding items to the backpack or equipping them.
	 */
	...
	/**
	 * Removes an item from the player's current position and also from the provided items iterator.
	 *
	 * @param playerPosition the current position of the player represented as a Room object
	 * @param items the iterator for the collection of items from which an item is to be removed
	 */
	...
	/**
	 * Handles the encounter between the player and enemies in the game. Checks if
	 * any enemy is in the same position as the player, triggering an attack. Also
	 * manages the player's decision to recover and updates the game state accordingly.
	 *
	 ...
	 * same position as the player. If an enemy is found, the player attacks the enemy using
	 * their firepower. If the enemy is killed, it is removed from the list, and the kill
	 * is logged. If the enemy survives, a message indicating the enemy endured the attack
	 * is appended to the confrontation information.
	 *
	 * After handling all confrontations, the method ensures no remaining enemies
	 * are present in the player's position and prints the confrontation's summary.
	 */
	...
	/**
	 * Checks if there are no enemies in the player's current position and updates the state
	 * of the player's position accordingly.
	 *
	 * @param playerPosition the current position of the player represented as a Room object.
	 */
	...
	/**
	 * Executes an attack by the player on an enemy. The enemy takes damage
	 * based on the player's attack strength, and relevant messages are displayed.
	 *
	...
	/**
	 * Handles the logic when one enemy dies during the game.
	 * Removes the enemy from its current position and updates the game state.
	 *
	 * @param enemy The enemy object that is being removed due to its death.
	 * @param enemies An iterator over the list of enemies, used to remove the deceased enemy.
	 */
	...
	/**
	 * Handles the scenario where the player decides to recover.
	 * This method transitions the game to the next scenario (ScenarioNr.FOUR) and
	 * executes the corresponding actions by calling scenarioQUATRO().
	 */
	...
	/**
	 * Handles the attack action where an enemy attacks a player, causing the player to take damage.
	 *
	 * @param enemy The enemy entity performing the attack. Contains information such as name and firepower.
	 * @param player The player entity being attacked. The player will receive damage based on the enemy's firepower.
	 */
	...
	/**
	 * Determines the best extraction point for the player based on the minimal path damage
	 * from the player's current position to each extraction point in the mission.
	 *
	 * @param playerPosition The current position of the player in the battlefield.
	 * @return The room representing the extraction point with the minimal damage path from the player's position.
	 * @throws ElementNotFoundException If no valid path or extraction point is found in the mission.
	 */
	...
	/**
	 * Calculates the total damage a player would take while traversing a given path.
	 * The method processes each room in the path and considers encounters with enemies
	 * and the effects of items like MediKits and Kevlar.
	 *
	 * @param path an iterator of Room objects representing the path to traverse.
	 *             Each room may contain enemies or items that affect the player's health.
	...
	/**
	 * Updates the weights of the battlefield edges for enemies.
	 * The method iterates over all rooms (vertices) in the battlefield graph and
	 * recalculates the weight of each edge connecting a room to its adjacent rooms.
	 * The new weights are evaluated using the calculateWeight method and are
	 * updated in the battlefield graph using the addEdge method.
	 *
	 * This process ensures that the graph accurately reflects the current gameplay
	 * state, particularly regarding enemy-related dynamics within the battlefield.
	 */
	...
	/**
	 * Calculates the weight of a given room based on the total damage inflicted by enemies present in the room.
	 *
	 * @param room the room for which the weight is to be calculated
	 * @return the calculated weight of the room based on the total firepower of enemies in the room
	 */
	 ...
	 * current state and whether the user is returning to the exit or proceeding to the next objective.
	 *
	 * @param isReturningToExit Indicates whether the mission is transitioning back to the exit point (true)
	 *                          or heading towards the next objective (false).
	 */
	...
	/**
	 * Displays the path from the starting room to the destination room.
	 * The path is calculated based on the shortest path between the provided rooms.
	 * If the starting or destination room is invalid or not found, an exception is thrown.
	 *
	 ...
	 * @throws ElementNotFoundException If the specified rooms do not exist or the path cannot be determined.
	 */
	 ...
	 *
	 * @param pathOutput a StringBuilder object that stores the formatted path information
	 * @param nextRoom the Room object representing the next room to evaluate
	 * @return the updated StringBuilder containing the appended information about the next room
	 */
	...
	/**
	 * Adds a room to the report based on the mission's target secured status.
	 * If the target is not secured, the room is added to the general report.
	 * If the target is secured, the room is added to the extraction section of the report.
	 *
	 ...
	 *
	 * @param returningToExtraction the boolean value to set; true if returning to extraction, false otherwise
	 */
