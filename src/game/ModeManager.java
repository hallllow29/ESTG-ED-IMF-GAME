 ...
 * automatic simulation modes. It also includes functionality to display mission and simulation
 * details such as rooms, connections, enemies, items, entry/exit points, and the target.
 */
public class ModeManager {

	/**
	 * Represents an instance of a Mission utilized within the ModeManager class
	 * to manage mission-related operations during the game's simulation or operation.
	 *
	 ...
	 * and manage key aspects of gameplay, including objectives, challenges, and resources.
	 */
	private Mission missionImpl;

	/**
	 * Default constructor for the ModeManager class. Initializes the instance
	 * of the ModeManager by setting its internal mission implementation to null.
	 * This class handles the selection of missions, game initialization, and
	 * simulation modes.
	 */
	public ModeManager() {
	...
	/**
	 * Creates a new player instance by gathering necessary input from the user.
	 * This method displays a welcome banner and prompts the user to enter a name
	 * and choose a backpack size. It then initializes and returns a Player object
	 * with the specified name, default firepower of 100, and the chosen backpack size.
	 *
	...
	/**
	 * Handles the mission selection process, allowing the user to choose from predefined missions.
	 * The chosen mission is loaded and set as the current mission implementation.
	 *
	 * This method uses a text-based menu to prompt the user to select a mission.
	 * Depending on the user's choice, it attempts to load the corresponding mission
	 * from a JSON file into the program's network structure.
	 *
	 * If an invalid option is selected, an error message is displayed, and no mission is loaded.
	 *
	 * @throws NotElementComparableException if the mission data includes elements that are not comparable,
	 *                                        making them incompatible with the program's requirements.
	 * @throws IOException if an I/O error occurs while reading the mission file.
	 * @throws ParseException if there is an error parsing the JSON structure of the mission file.
	 */
	...
	/**
	 * Initializes and starts the game by managing the simulation mode selection and handling the game flow accordingly.
	 *
	 ...
	 * 3. Displays a menu to the user for selecting the type of simulation mode (automatic or manual).
	 * 4. Based on the user's choice, triggers either the automatic or manual simulation by invoking the respective methods.
	 ...
	 *
	 * @throws NotElementComparableException if the selected mission contains non-comparable elements.
	 * @throws IOException if an I/O error occurs during mission selection.
	 * @throws ParseException if the mission configuration file contains parsing errors.
	 * @throws ElementNotFoundException if an element required for the simulation is not found.
	 * @throws EmptyCollectionException if an operation on a collection is attempted but the collection is empty.
	 */
	 ...
	 * and executes the game logic for the automatic mode. Once the simulation is complete,
	 * it saves the simulation report to a JSON file.
	 *
	 * @param player the player participating in the automatic simulation process
	 * @throws ElementNotFoundException if a required element is not found during the simulation
	 * @throws EmptyCollectionException if an operation on a collection in the simulation encounters an empty collection
	 */
	...
	/**
	 * Executes the manual simulation process for the player in the current mission.
	 * This method creates a report for the simulation, initializes manual mode,
	 * and allows the user to interactively play through the mission. Once the simulation
	 * is complete, it saves the simulation report to a JSON file.
	 *
	 ...
	 *
	 * This method aggregates and calls several auxiliary display methods
	 * to provide a comprehensive overview of the mission. The information
	 * includes the mission code, details of the rooms in the battlefield,
	 * connections between rooms, enemy intelligence, available items,
	 * entry and exit points, and the mission's target.
	 *
	 * The method performs the following operations sequentially:
	 * 1. Displays the mission code using the `displayMission` method.
	 * 2. Shows the rooms that exist in the mission's battlefield by calling `displayRoomDetails`.
	 * 3. Displays details of connections between the rooms by calling `displayAdjacentRoomDetails`.
	 ...
	 *
	 * This method prints a formatted header followed by the mission's code,
	 * retrieved from the mission implementation's `getCode` method. It provides
	 * a concise way to display the mission identifier to the user.
	 */
	...
	/**
	 * Displays the details of the rooms in the given graph structure.
	 * Iterates through the vertices of the graph, which represent rooms,
	...
	/**
	 * Displays the details of adjacent room connections in a given graph structure.
	 * For each room in the graph, this method iterates through its connected rooms
	 * and displays their connections by printing a formatted string to the console.
	 *
	 ...
	 *
	 * This method retrieves a list of enemies from the current mission implementation
	 * and iterates through each enemy to compile their details into a formatted string.
	 * The details displayed for each enemy include:
	 * - Name
	 * - Fire Power
	 * - Position
	 *
	 * After collecting the information, it prints the formatted enemy intelligence
	 * to the console. This method helps provide an overview of enemy attributes and
	 * positions within the mission's context.
	 */
	 ...
	 * iterates through each item, and compiles their details into a formatted string.
	 * The details displayed for each item include:
	 * - Name
	 * - Value
	 * - Position
	 *
	 * The collected information is then printed to the console, providing
	 * an overview of the items present in the mission for the user's reference.
	 */
	 ...
	 *
	 * This method retrieves the list of rooms designated as entry
	 * and exit points for the mission from the mission implementation.
	 * It then iterates through the list and prints the names of these
	 * rooms to the console. The output is accompanied by a header
	 * indicating that the displayed rooms are entry and exit points.
	 */
	...
	/**
	 * Allows the user to choose a backpack size through a console-based selection process.
	 * This method displays a banner and options for backpack selection, accepts the user's choice,
	 * and returns a `BackPack` object initialized with the chosen size's capacity.
	 *
	 * @param scanner the Scanner object used to read the user's input from the console
	 * @return a BackPack object initialized with the capacity of the selected backpack size
	 */
